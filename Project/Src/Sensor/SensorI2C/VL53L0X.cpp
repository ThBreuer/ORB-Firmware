...
#define ADDRESS_DEFAULT 0x29 // 7 bit address
...
VL53L0X::VL53L0X( I2Cmaster &i2cIn, /*cTimer &timerIn,*/ BYTE addrOffset  )
  : //address(ADDRESS_DEFAULT<<1)
   io_timeout(0) // no timeout
  , did_timeout(false)
  , i2c( i2cIn, (ADDRESS_DEFAULT+addrOffset)<<1 )
  //, timer( In )
{
}
...
void VL53L0X::setAddress(uint8_t addrOffset)
{
  writeReg(I2C_SLAVE_DEVICE_ADDRESS, ADDRESS_DEFAULT+addrOffset);
//  address = new_addr;
}
...
//  last_status = Wire.endTransmission();
  WORD data = value; // todo check byte order {(BYTE)(value>>8),(BYTE)(value&0xFF)};
  i2c.write(reg, data);
  last_status = i2c.isError();
}
...
//  last_status = Wire.endTransmission();
  BYTE data[4] = {(BYTE)((value >> 24) & 0xFF),
                  (BYTE)((value >> 16) & 0xFF),
                  (BYTE)((value >>  8) & 0xFF),
                  (BYTE)((value      ) & 0xFF) };

  i2c.write(reg, value); //data,4); // todo check byte order
  last_status = i2c.isError();
}
...
//  value |=           Wire.read();      // value low byte
  ...
  return ( ((WORD)data<<8) | data&0xFF ); // check byte order
}
...
//  value |=           Wire.read();       // value lowest byte
  ...
  //BYTE data[4];
  WORD data;
  i2c.read(reg,&data); // todo check byte order
  last_status = i2c.isError();
...
{
  timer.start( io_timeout );
  while ((readReg(RESULT_INTERRUPT_STATUS) & 0x07) == 0)
  {
    if (timer.timeout() /*checkTimeoutExpired()*/)
    {
      did_timeout = true;
      return 65535;
    }
